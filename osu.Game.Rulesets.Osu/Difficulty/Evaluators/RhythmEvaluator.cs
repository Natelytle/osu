...
using osu.Game.Rulesets.Osu.Difficulty.Functions;
    ...
    public class RhythmEvaluator
    {
        ...
        private static double evaluateDifficultyOf(EvenHitObjects evenHitObjects, double hitWindow)
        {
            ...
            if (previousInterval != null && evenHitObjects.Children.Count > 1)
            {
                ...
                intervalDifficulty *= MathFunctions.Sigmoid(durationDifference / hitWindow, 1.5, 0.5, 0.5, 1);
            }

            // Penalize patterns that can be hit within a single hit window.
            intervalDifficulty *= MathFunctions.Sigmoid(evenHitObjects.Duration / hitWindow, 1, 0.5, 0.5, 1);
        ...
        {
            OsuDifficultyHitObjectRhythm rhythm = ((OsuDifficultyHitObject)hitObject).Rhythm;
