...
{
    public class PressingEvaluatorTest
    {
        ...
        {
            ManiaDifficultyHitObject? prev = current.PrevHead(0);
            ...
            double delta = current.HeadDeltaTime;
            double baseDifficulty = 1000.0 / delta;

            // Multiplier based on how many long notes are currently being held.
            double longNoteBonus = 1.0;

            if (prev is not null)
            {
                longNoteBonus += 6 * lnHeldSecondsBetween(prev.StartTime, current.StartTime, prev.PreviousHeadObjects);
            }

            // A bonus for notes with a length between stream_bpm_start and stream_bpm_end.
            double streamBonus = 1.0;
            double streamBpm = DifficultyCalculationUtils.MillisecondsToBPM(delta);
            ...
            if (delta < (2.0 / 3.0) * leniency)
            {
                double deviation = (delta - leniency / 2.0) / 1000.0;
