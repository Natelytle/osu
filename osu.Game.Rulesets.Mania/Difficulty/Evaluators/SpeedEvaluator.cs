...
namespace osu.Game.Rulesets.Mania.Difficulty.Evaluators
{
    public class SpeedEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current)
        {
            ...
            double noteDelta = maniaCurrObj.StartTime - maniaCurrObj.PreviousHitObjects.ToList().ConvertAll(obj => (obj?.StartTime ?? double.PositiveInfinity) + grace_note_tolerance).Min();
            ...
            return 0; // speed_factor * speedBpmScale(speedBpm) * staminaBonus;
        }

        private static ManiaDifficultyHitObject? getPreviousChord(ManiaDifficultyHitObject note)
        {
            return note.PreviousHitObjects.OrderBy(obj => obj?.StartTime).LastOrDefault();
        }
