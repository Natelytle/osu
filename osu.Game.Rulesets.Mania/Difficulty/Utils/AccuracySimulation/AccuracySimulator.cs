...
namespace osu.Game.Rulesets.Mania.Difficulty.Utils.AccuracySimulation
{
    public class AccuracySimulator
    {
        ...
        // One note is removed just to ensure everything stays smooth and chill
        private double maxScore => (noteDifficulties.Count + tailDifficulties.Count - 1) * MAX_JUDGEMENT_WEIGHT;
        ...
        public double SkillLevelAtAccuracy(double accuracy)
        {
            ...
            double skill = RootFinding.FindRootExpand(skill => accuracyProb(accuracy, skill) - accuracy_prob, 0, Math.Max(maxNoteDifficulty, maxTailDifficulty) * 2);

            return skill;
        }

        /// <summary>
        /// The probability of achieving x accuracy given y skill.
        /// </summary>
        ...
        private double accuracyProb(double accuracy, double skill)
        {
            ...
            IPoissonBinomial poissonBinomial = scoreDistributionAt(skill);

            double prob = 1 - poissonBinomial.CDF(accuracy * maxScore);

            return prob;
        }

        /// <summary>
        /// The percentage accuracy achieved at skill.
        /// </summary>
        /// <param name="skill"></param>
        private double accuracyAt(double skill)
        {
            ...
            double accuracy = RootFinding.FindRootExpand(accuracy => 1 - poissonBinomial.CDF(accuracy * maxScore) - accuracy_prob, 0, 1);

            return accuracy;
        }

        private IPoissonBinomial scoreDistributionAt(double skill)
        {
            IterativePoissonBinomial poissonBinomial = new IterativePoissonBinomial();
            ...
            if (noteDifficulties.Count < bin_threshold)
            {
                for (int i = 0; i < noteDifficulties.Count; i++)
                {
                    ...
                    poissonBinomial.AddJudgementProbabilities(noteProbs);
                }
                ...
                for (int i = 0; i < tailDifficulties.Count; i++)
                {
                    ...
                    poissonBinomial.AddJudgementProbabilities(tailProbs);
                }
            }
            else
            {
                for (int i = 0; i < binNotes.Count; i++)
                {
                    ...
                    poissonBinomial.AddJudgementProbabilities(noteProbs, binNotes[i].Count);
                }
                ...
                for (int i = 0; i < binTails.Count; i++)
                {
                    ...
                    poissonBinomial.AddJudgementProbabilities(tailProbs, binTails[i].Count);
                }
            }
